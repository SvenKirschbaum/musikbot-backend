...
function parseupdate(data) {
	...
	data.playlist.forEach(function(e) {
		...
		if($(".admin").length == 0) {
			var row = $('<tr style="display:none" class="songentry" id="song_'+e.id+'"><td style="text-align: center">'+e.id+'</td><td>'+("0" + date.getDate()).slice(-2)+'.'+("0" + (date.getMonth()+1)).slice(-2)+'.'+date.getFullYear()+' - '+("0" + date.getHours()).slice(-2)+':'+("0" + date.getMinutes()).slice(-2)+':'+("0" + date.getSeconds()).slice(-2)+'</td><td><img alt="pb_playlist" src="https://www.gravatar.com/avatar/'+e.gravatarid+'?s=20"> '+e.autor+'</td><td title="'+e.title+'">'+shorttitle+'</td><td><a href="'+e.link+'" target="_blank">'+e.link+'</a></td></tr>');
		}
		else {
			var row = $('<tr style="display:none" class="songentry" id="song_'+e.id+'"><td style="text-align: center" class="draghandle">'+e.id+'</td><td>'+("0" + date.getDate()).slice(-2)+'.'+("0" + (date.getMonth()+1)).slice(-2)+'.'+date.getFullYear()+' - '+("0" + date.getHours()).slice(-2)+':'+("0" + date.getMinutes()).slice(-2)+':'+("0" + date.getSeconds()).slice(-2)+'</td><td title="'+e.autor+'"><img alt="pb_playlist" src="https://www.gravatar.com/avatar/'+e.gravatarid+'?s=20"> '+guestname(e.autor)+'</td><td title="'+e.title+'">'+shorttitle+'</td><td><a href="'+e.link+'" target="_blank">'+e.link+'</a></td><td><input type="checkbox" name="song" value="'+e.id+'"></td></tr>');
		}
		row.hide();
		$('#playlist').append(row);
		row.fadeIn("slow");
