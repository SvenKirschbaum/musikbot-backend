...
public class ConnectionHandler {
	...
	public void run() {
		...
		if (authcmd.getCmd() != Command.AUTH
				|| !authcmd.getdata().equals(this.authkey)) {
			try {
				Logger.getLogger(ConnectionHandler.class).warn(
						"Authentification Failure, clsing connection!");
			...
			try {
				...
				this.out.flush();
				Logger.getLogger(ConnectionHandler.class).info(
		...
		while (!this.sock.isClosed() && this.sock.isConnected()) {
			try {
				...
				Logger.getLogger(ConnectionHandler.class).debug(
						"Got CMD: " + cmd.getCmd());
				switch (cmd.getCmd()) {
				case Command.EMPTY:
				...
				case Command.REQUEST_SONG: {
					...
					if (song != null) {
						...
						this.out.writeObject(new Command(
								Command.NO_SONG_AVAILABLE));
						this.out.flush();
				...
				case Command.PLAYBACK_ERROR: {
					/*Logger.getLogger(ConnectionHandler.class).error(
							"Client reported PLAYBACK_ERROR: "+(String)cmd.getdata());
					Song song = this.getController().getnextSong();
					if (song != null) {
						this.sendSong(song);
					} else {
				...
				Logger.getLogger(ConnectionHandler.class).debug(
						"Die Verbindung wurde vom Client getrennt!", e);
				break;
