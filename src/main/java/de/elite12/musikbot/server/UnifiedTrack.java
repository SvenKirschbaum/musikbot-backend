...
public class UnifiedTrack {
	...
	private UnifiedTrack(Song s) throws IOException, TrackNotAvailableException, InvalidURLException {
		this.song = s;
		String VID = Util.getVID(s.getLink());
		String SID = Util.getSID(s.getLink());
		...
		if (VID != null) {
			...
			try {
				this.track = Util.getTrackRaw(Util.getSID(s.getLink()));
	...
	public String getId() {
		if (this.type == Type.YOUTUBE) {
			return Util.getVID(this.song.getLink());
		}
		if (this.type == Type.SPOTIFY) {
			return Util.getSID(this.song.getLink());
		}
		throw new RuntimeException("Invalid State");
	}
