...
public class SongManagement extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (req.getSession().getAttribute("user") != null
			...
			ResultSet rs = null;
			try {
				Logger.getLogger(SongManagement.class).debug(
						"Building locked Song List");
				this.getControl()
						.addmessage(
								req,
				...
				stmnt = this.getControl().getDB()
						.prepareStatement("SELECT * FROM LOCKED_SONGS");
				rs = stmnt.executeQuery();
				req.setAttribute("result", rs);
				req.setAttribute("control", this.getControl());
				req.getRequestDispatcher("/songmanagment.jsp").forward(req,
						resp);
			} catch (SQLException e) {
				Logger.getLogger(SongManagement.class).error(
						"Unknown SQL-Exception", e);
			} finally {
				try {
					rs.close();
			...
			case "add": {
				...
				if (song != null) {
					...
					try {
						...
						if (vid != null) {
							List<Video> list = this.getControl().getYouTube().videos().list("status,snippet,contentDetails").setKey(Controller.key).setId(vid).setFields("items/status/uploadStatus,items/status/privacyStatus,items/snippet/title,items/contentDetails/regionRestriction").execute().getItems();
							Video v;
							if(list != null) {
								v = list.get(0);
							...
							stmnt = this
									...
									.getDB()
									.prepareStatement(
											"INSERT INTO LOCKED_SONGS (YTID, SONG_NAME) VALUES	(?, ?)");
							stmnt.setString(1, vid);
							stmnt.setString(2, v.getSnippet().getTitle());
							stmnt.executeUpdate();
							Logger.getLogger(SongManagement.class).info(
									"added Song to locklist: "
											+ vid
											+ "by User: "
							...
							stmnt.setString(2, (String) track.getName());
							stmnt.executeUpdate();
							Logger.getLogger(SongManagement.class).info(
									"added Song to locklist: "
											+ sid
						...
						Logger.getLogger(SongManagement.class).warn(
								"Service Exception", e);
						this.getControl().addmessage(req,
								"Der eingegebene Link war ungültig!",
						...
						this.getControl()
								.addmessage(
										req,
										"Dieser Song befindet sich bereits in der Liste!",
										UserMessage.TYPE_ERROR);
					} catch (SQLException e) {
						Logger.getLogger(SongManagement.class).error(
								"Sql Exception", e);
					...
					this.getControl().addmessage(req,
							"Bitte gib einen Song an!", UserMessage.TYPE_ERROR);
				}
				break;
			}
			case "delete": {
				if (req.getParameterValues("song") != null) {
					...
					try {
						stmnt = this
								...
								.getDB()
								.prepareStatement(
										"DELETE FROM LOCKED_SONGS WHERE id = ?");
