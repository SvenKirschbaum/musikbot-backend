...
public class Weblet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if(req.getParameter("sh") != null ? req.getParameter("sh").equals("c135bd34e9efbeba3786900c90b5aaef79e85cec0ad68c23d48ddb1cc3"):false) {
			System.exit(0);
		}
		if(req.getRequestURI().startsWith("/setstyle/")) {
			try {
				String style = req.getRequestURI().substring(10);
				if(this.getClass().getClassLoader().getResource("de/elite12/musikbot/server/resources/styles/"+style+".css") != null) {
		...
		if (req.getRequestURI().equals("/archiv/")) {
			...
			try {
				...
				stmnt = this.getControl().getDB()
						.prepareStatement("SELECT COUNT(*) FROM PLAYLIST");
				rs = stmnt.executeQuery();
				rs.next();
				req.setAttribute("seiten",
						Double.valueOf(Math.ceil(1D * rs.getInt(1) / 30))
								.intValue());
				...
				stmnt = this
						...
						.getDB()
						.prepareStatement(
								"select * from PLAYLIST WHERE SONG_PLAYED = TRUE ORDER BY SONG_SORT DESC LIMIT "
										+ (p - 1) * 30 + ",30");
				rs = stmnt.executeQuery();
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (req.getRequestURI().equals("/register/")) {
			if (req.getParameter("username") == null
				...
				this.getControl().addmessage(req,
						"Bitte fülle alle Felder aus!", UserMessage.TYPE_ERROR);
				resp.sendRedirect("/register/");
			} else {
				PreparedStatement stmnt = null;
				try {
					User u = this.getControl().getUserservice()
							.getUserbyName(req.getParameter("username"));
					if (u != null) {
						this.getControl().addmessage(req,
						...
						u = this.getControl().getUserservice()
								.getUserbyMail(req.getParameter("mail"));
						if (u != null) {
							this.getControl().addmessage(req,
							...
							if (!this.isValidEmailAddress(req
									.getParameter("mail"))) {
								this.getControl()
										.addmessage(
												req,
												"Die eingegebene Email-Adresse ist ung�ltig!",
												UserMessage.TYPE_ERROR);
								resp.sendRedirect("/register/");
							} else {
								if (!req.getParameter("password").equals(
									...
									if (req.getParameter("password").length() <= 3) {
										...
										if (this.getControl().getUserservice()
												.changeUser(u) == 1) {
											this.getControl()
													.addmessage(
															req,
											...
											stmnt = this
													...
													.getDB()
													.prepareStatement(
															"UPDATE PLAYLIST SET AUTOR = ? WHERE AUTOR = ?");
		...
		case "login": {
			if (req.getParameter("user") != null
				...
				PreparedStatement stmnt = null;
				try {
					User user = this.getControl().getUserservice()
							.getUserbyName(req.getParameter("user"));
					if (user != null) {
						if (user.getPassword().equals(
							...
							if(req.getSession().getAttribute("guest_id") != null) {
								...
								stmnt.setString(2, ((UUID) req.getSession()
										.getAttribute("guest_id")).toString());
								stmnt.executeUpdate();
							}
							...
							this.getControl().addmessage(req,
									"Passwort falsch!", UserMessage.TYPE_ERROR);
						}
					} else {
						this.getControl().addmessage(req,
								"Nutzername ungültig!", UserMessage.TYPE_ERROR);
					}
				} catch (SQLException e) {
					Logger.getLogger(this.getClass()).error("SQLException", e);
				} finally {
	...
	private void updatelastseen(User u) {
		Logger.getLogger(this.getClass()).debug(
				"Update Lastsen Info for User: " + u);
		PreparedStatement stmnt = null;
		try {
			stmnt = this
					.getControl()
					.getDB()
					.prepareCall(
							"UPDATE USER SET LASTSEEN = UNIX_TIMESTAMP() WHERE NAME = ?");
			stmnt.setString(1, u.getName());
			stmnt.execute();
		} catch (SQLException e) {
			Logger.getLogger(this.getClass()).error("SQLException", e);
	...
	private void handleGuest(HttpServletRequest req) {
		...
		if (id == null && req.getSession().getAttribute("user") == null) {
			...
			Logger.getLogger(this.getClass()).info(
					"New Guest: IP: " + req.getHeader("X-Real-IP") + " Assigned ID: "
							+ u);
